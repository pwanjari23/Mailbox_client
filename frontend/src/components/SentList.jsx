...
import { ListGroup, Card, Alert, Spinner, Modal, Button } from 'react-bootstrap';
...
const SentList = () => {
  ...
  const fetchEmails = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/emails/sent`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to retrieve sent emails.');
      }

      setEmails(data.emails || []);
    } catch (err) {
      console.error('Sentbox fetch error:', err);
      ...
      <Card.Body className="p-2 py-3">
        <div className="d-flex align-items-center justify-content-between mb-4 pb-2 border-bottom border-secondary">
          <div className="d-flex align-items-center">
            <i className="bi bi-send-fill me-2 text-indigo" style={{ fontSize: '1.5rem', color: '#818cf8' }}></i>
            <h2 className="signup-title mb-0" style={{ fontSize: '1.5rem' }}>Sent Messages</h2>
          </div>
          <Button 
        ...
        {loading ? (
          <div className="text-center py-5">
            ...
            <p className="signup-subtitle">Loading sent messages...</p>
          </div>
        ) : emails.length === 0 ? (
          <div className="text-center py-5 px-3">
            ...
            >
              <i className="bi bi-send" style={{ fontSize: '2rem', color: '#64748b' }}></i>
            </div>
            <h4>No sent messages</h4>
            <p className="signup-subtitle mb-0">Emails you compose and send will appear here.</p>
          </div>
        ) : (
              ...
              <Modal.Body className="py-4">
                <div className="mb-4 pb-3 border-bottom border-secondary" style={{ fontSize: '0.9rem' }}>
                  ...
                  <div>
                    ...
                    <span style={{ color: '#94a3b8' }}>{selectedEmail.senderEmail} (You)</span>
                  </div>
                </div>
